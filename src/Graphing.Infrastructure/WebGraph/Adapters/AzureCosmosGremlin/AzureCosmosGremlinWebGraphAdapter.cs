...
{
    public class AzureCosmosGremlinWebGraphAdapter : BaseWebGraph
    {
        private readonly GremlinClient _gremlinClient;
        private readonly IGremlinQueryProvider _gremlinQueryProvider;

        public AzureCosmosGremlinWebGraphAdapter(ILogger logger, 
            GraphingSettings graphingSettings, AzureCosmosGremlinSettings azureCosmosSettings)
            : base(logger, graphingSettings)
        {
            try
            {
                var gremlinServer = new GremlinServer(
                    azureCosmosSettings.Hostname,
                    azureCosmosSettings.Port,
                    azureCosmosSettings.EnableSsl,
                    username: $"/dbs/{azureCosmosSettings.Database}/colls/{azureCosmosSettings.Graph}",
                    password: azureCosmosSettings.PrimaryKey
                );
            ...
            _gremlinQueryProvider = new GremlinQueryProvider(
                logger, 
                _gremlinClient, 
                azureCosmosSettings.MaxQueryRetries);
        }
