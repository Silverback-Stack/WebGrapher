...
namespace Graphing.Infrastructure.WebGraph.Serializers.SigmaJs
{
    ...
    {
        private readonly SigmaJsGraphSettings _settings;

        public SigmaJsGraphSerializer(SigmaJsGraphSettings settings) { 
            _settings = settings;
        }

        public (IReadOnlyList<SigmaGraphNodeDto> Nodes, IReadOnlyList<SigmaGraphEdgeDto> Edges)
        ...
        private SigmaGraphNodeDto CreateNodeDto(Node node) => 
        ...
        private SigmaGraphEdgeDto CreateEdgeDto(string sourceUrl, string targetUrl)
        {
        ...
        private double CalculateNodeSize(int popularityScore)
        {
            var minSize = _settings.MinNodeSize;
            var maxSize = _settings.MaxNodeSize;
